...
func TestMySQL(t *testing.T) {
	...
	dbHost := os.Getenv("MIGU_DB_MYSQL_HOST")
	if dbHost == "" {
		dbHost = "localhost"
	}
	...
	exec := func(queries []string) (err error) {
		tx, err := db.Begin()
		if err != nil {
			return err
		}
	...
	t.Run("Diff", func(t *testing.T) {
		...
		t.Run("with src", func(t *testing.T) {
			before(t)
			testDiffWithSrc := func(t *testing.T, t1, s1, t2, s2 string) {
				d := dialect.NewMySQL(db)
				src := fmt.Sprintf("package migu_test\n"+
					"//+migu\n"+
					"type User struct {\n"+
